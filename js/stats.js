...
export const stats = {
    render(mode, btn) {
            ...
            Object.values(pulls).forEach(pData => {
                if (pData.details && pData.details.length > 0) {
                    pData.details.forEach(det => {
                        if (det.type === 'pickup') { accountStats[record.account].pickup++; globalStats.pickup++; }
                        if (det.type === 'spook') { accountStats[record.account].spook++; globalStats.spook++; }
                        if (det.type === 'pilgrim') { accountStats[record.account].pilgrim++; globalStats.pilgrim++; }
        ...
        // 카드 생성 함수
        const createStatCard = (title, data) => {
            const totalSSR = data.pickup + data.spook + data.pilgrim;
            const ssrRate = data.pulls > 0 ? ((totalSSR / data.pulls) * 100).toFixed(2) : 0;
            
            return `
              <div class="card" style="margin-bottom: 15px;">
                <h3 style="margin-top: 0; font-size: 1.1rem; border-bottom: 2px solid var(--border); padding-bottom: 10px;">${title}</h3>
